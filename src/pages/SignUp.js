...
const SignUp = () => {
  ...
  const onSubmit = ({ email, password, name }) => {
    setLoading(true);
    AdminServices.addStaff({ staffData: { email: email, password: password, name: name } })
      .then((res) => {
        if (res) {
          setLoading(false);
          notifySuccess('Account Creation Success!');
          history.replace('/');
        }
      })
      .catch((err) => {
          ...
          <main className="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
            <div className="w-full">
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <Button block className="mt-4 h-12"
                  to="/login" disabled={loading} type="submit">
                  Create account
                </Button>
